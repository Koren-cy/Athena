...
> [!abstract] 本章定位
> 本章讨论模拟调制的原理和抗噪声性能。幅度调制共四种（AM、DSB、SSB、VSB），角度调制两种（FM、PM），最后介绍频分复用。重点：各调制的表达式、带宽、功率、调制制度增益，以及卡森公式。
...
1. **匹配信道特性，减小天线尺寸**：天线尺寸 $h$ 一般要求 $\geq \lambda/10$。由 $c = \lambda f$，若 $f = 3000$ Hz，则 $\lambda = 10^5$ m，$h \geq 10^4$ m，无法实际实现。若提高到 $f = 900$ MHz，则 $\lambda \approx 0.33$ m → 天线尺寸降至厘米级。
...
载波表达式：$c(t) = A\cos(\omega_c t + \varphi)$。用 $m(t)$ 控制 $A$ → 幅度调制；控制 $\omega_c$ → 频率调制（FM）；控制 $\varphi$ → 相位调制（PM）。FM 和 PM 统称**角度调制**——因为都表现为对 $\omega_c t + \varphi$ 这一整体角度的控制。

从波形上直观区分：**调幅**——$m(t)$ 蕴含在载波包络的变化中；**调频**——$m(t)$ 蕴含在载波瞬时频率的变化中。
...
直流偏置 $A_0$ 的作用是把 $m(t)$ 抬升到时间轴上方，再乘以极性交变的载波。当 $a \leq 1$ 时，包络与 $m(t)$ 成正比 → 可用包络检波。当 $a > 1$ 时，$A_0$ 不足以完全抬升，出现**载波反向点**（相位 180° 突变）→ 包络不再正比于 $m(t)$ → 不能包络检波。
...
由 $A_0 \geq |m(t)|_{\max}$ 可证 $\eta_{\text{AM}} \leq 50\%$。载波分量不携带信息，却消耗了大量功率，导致 AM 的调制效率很低。

**单音调制特例**：$m(t) = A_m\cos\omega_m t$，则 $\overline{m^2(t)} = A_m^2/2$，调幅系数 $a = A_m/A_0$。

$$\eta_{\text{AM}} = \frac{a^2}{2 + a^2}$$

满调幅（$a = 1$）时 $\eta_{\max} = 1/3$——单音满调幅的效率仅为 33%。

边带功率与载波功率的关系：$P_s = \dfrac{1}{2}a^2 P_c$。

#### 解调：包络检波

仅适用于 AM 信号。利用二极管与 RC 电路提取包络，再通过隔直电容去除直流偏置 $A_0$，即可恢复 $m(t)$。条件是 $a \leq 1$（不过调幅）。
...
**VSB 滤波器的条件**（相干解调后要无失真恢复 $m(t)$，需满足）：已调信号 $s_{\\text{VSB}}(t)$ 经相干解调（$\\times\\cos\\omega_c t$ → LPF）后，输出频谱为 $\\frac{1}{4}M(\\omega)[H_{\\text{VSB}}(\\omega+\\omega_c) + H_{\\text{VSB}}(\\omega-\\omega_c)]$。为使输出等于 $\\frac{1}{4}M(\\omega)$（无失真），必须：

$$H_{\text{VSB}}(\omega + \omega_c) + H_{\text{VSB}}(\omega - \omega_c) = \text{常数}, \quad |\omega| \leq \omega_H$$

即在 $\pm\omega_c$ 附近，滤波器的幅度响应呈**互补对称**（奇对称滚降）——这使滤波器可以有一个渐变的过渡带，不要求理想陡峭的截止特性。

### 相干解调

适用于所有幅度调制（DSB、SSB、VSB 必须用，AM 也可用）。

**步骤**：已调信号与相干载波 $\cos\omega_c t$ 相乘 → 低通滤波（LPF）。
...
- BPF：带宽等于已调信号带宽，中心频率为载频，起**传信滤噪**的作用
...
> 相干解调过程中，噪声的正交分量 $n_s(t)$ 被 LPF 滤除，而信号本身没有正交分量 → 信噪比改善一倍。
...
> **不！** 比较抗噪声性能必须在 **$S_i$ 相等、$n_0$ 相等、$f_H$ 相等** 的前提下进行。在此前提下：
...
>
> **解**：
> $\overline{m^2(t)}$ = 功率谱下的面积 = $2 \times (\frac{1}{2} \times f_m \times \alpha)$（两个三角形，底边 $f_m$、高 $\alpha$）= $f_m \alpha$。
>
> $S_i = \frac{1}{2}\overline{m^2(t)} = \frac{1}{2}f_m\alpha$
>
> $S_o = \frac{1}{4}\overline{m^2(t)} = \frac{1}{4}f_m\alpha$
>
> $N_i = n_0 B = n_0 \cdot 2f_m$，$N_o = \frac{1}{4}N_i = \frac{1}{2}n_0 f_m$
>
> $\dfrac{S_o}{N_o} = \dfrac{\frac{1}{4}f_m\alpha}{\frac{1}{2}n_0 f_m} = \dfrac{\alpha}{2n_0}$
...
虽然理论带宽无限，但 $J_n(m_f)$ 随 $n$ 增大而收敛。通常只保留 $n \leq m_f+1$ 的边频分量。
...
**结论**：FM 信号的总平均功率等于未调载波功率。调制只改变功率在载波和各边频分量之间的**分配**（各边频分量的功率分配取决于 $m_f$），不改变总功率。
